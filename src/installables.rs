...
impl From<&str> for Installable {
    fn from(value: &str) -> Self {
        ...
        Installable::new_flake(x.0, &x.1.split('.').collect::<Vec<_>>())
    }
}

impl Installable {
    pub fn new_flake<S>(reference: S, attribute: &[S]) -> Self
    where
        S: AsRef<str>,
    {
    ...
    pub fn to_args(&self) -> Vec<String> {
        ...
        match &self {
            ...
            Installable::NixFile(inner) => {
                res.push("-f".to_string());
                res.push(inner.path.clone());
                // FIXME add -A
            }
        }
...
#[test]
fn test_display() {
    let installable = Installable::new_flake(".", &["foo", "bar.local", "baz"]);
